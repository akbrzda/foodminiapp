...
const DEFAULT_ADMIN_REFRESH_SESSION_HOURS = 12;
const MAX_ADMIN_REFRESH_SESSION_HOURS = 24 * 30;
const resolveAdminRefreshSessionHours = () => {
  const rawHours = Number.parseInt(process.env.ADMIN_REFRESH_SESSION_HOURS || "", 10);
  if (!Number.isFinite(rawHours) || rawHours < 1) {
    return DEFAULT_ADMIN_REFRESH_SESSION_HOURS;
  }
  return Math.min(rawHours, MAX_ADMIN_REFRESH_SESSION_HOURS);
};
const ADMIN_REFRESH_SESSION_HOURS = resolveAdminRefreshSessionHours();
const CLIENT_REFRESH_TOKEN_TTL = "7d";
const CLIENT_REFRESH_TOKEN_COOKIE_MAX_AGE = 7 * 24 * 60 * 60 * 1000;
const ADMIN_REFRESH_TOKEN_TTL = `${ADMIN_REFRESH_SESSION_HOURS}h`;
const ADMIN_REFRESH_TOKEN_COOKIE_MAX_AGE = ADMIN_REFRESH_SESSION_HOURS * 60 * 60 * 1000;
