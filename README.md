...
Схема для деплоя чистого проекта — `backend/database/schema.sql` (отсортирована по зависимостям и содержит безопасное включение/выключение FK‑проверок).
...
- `admin_users.eruda_enabled` — флаг включения Eruda (debug‑консоль) для администраторов с Telegram ID в mini app.
...
- В карточке администратора доступен переключатель Eruda (только при наличии Telegram ID), действует в mini app.
