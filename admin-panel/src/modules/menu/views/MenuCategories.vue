    ...
    <Card>
      <CardContent class="!p-0">
        <div class="space-y-3 p-3 md:hidden">
          ...
          <template v-else>
            <div v-for="category in paginatedCategories" :key="`mobile-${category.id}`" class="rounded-xl border border-border bg-background p-3">
              <div class="font-medium text-foreground">{{ category.name }}</div>
              <div class="text-xs text-muted-foreground">{{ category.description || "—" }}</div>
              <div class="mt-2 text-xs text-muted-foreground">Порядок: {{ formatNumber(category.sort_order || 0) }}</div>
              <div class="mt-2">
                <Badge
                  variant="secondary"
