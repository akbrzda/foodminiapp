<template>
  <div ref="rootRef" :class="cn('relative w-full', props.class)">
    ...
    <div
      v-if="open"
      class="absolute right-0 z-20 mt-2 w-[680px] max-w-[calc(100vw-2rem)] rounded-md border border-border bg-card p-3 shadow-lg"
    >
...
const weekdayLabels = Array.from({ length: 7 }, (_, index) =>
  weekdayFormatter.format(addDays(weekdayBase, index)).replace(".", "")
);
...
const monthDays = (monthDate) => {
  ...
  for (let cursor = start; cursor <= end; cursor = addDays(cursor, 1)) {
    ...
    const inRange =
      fromDate.value && toDate.value && (isAfter(cursor, fromDate.value) || isStart) && (isBefore(cursor, toDate.value) || isEnd);
