...
import { sendMessage as sendMaxMessage } from "../../services/maxApi.js";
...
const defaultMaxApi = {
  sendMessage: sendMaxMessage,
};

export const createBotApiServer = ({ commandBot, telegramApi = defaultTelegramApi, maxApi = defaultMaxApi, sendStartMessage = sendTelegramStartMessage }) => {
  const app = express();

  app.use(express.json({ limit: "1mb" }));
  app.use(createHealthRoutes());
  app.use(createWebhookRoutes({ commandBot }));
  app.use(createInternalRoutes({ telegramApi, maxApi, sendStartMessage }));
